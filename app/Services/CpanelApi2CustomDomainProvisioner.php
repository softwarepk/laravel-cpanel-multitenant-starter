...
class CpanelApi2CustomDomainProvisioner implements CustomDomainProvisioner
{
    ...
    public function ensureCustomDomainReady(string $domain): void
    {
        ...
        if ($domain === '') throw new RuntimeException('Custom domain cannot be empty.');
        ...
        if ($platformDomain === '' || $configuredRoot === '') throw new RuntimeException('The platform domain and document root must be configured.');
        $platformData = $this->domainData($platformDomain);
        if ($platformData === null) throw new RuntimeException('The configured platform domain is not registered in cPanel.');
        ...
        if ($existing !== null) { $this->assertDocumentRoot($domain, $existing, $expectedRoot); return; }
        ...
        $created = null;
        for ($attempt = 0; $attempt < 4; $attempt++) { $created = $this->domainData($domain); if ($created !== null) break; usleep(250000); }
        if ($created === null) throw new RuntimeException("cPanel reported success, but {$domain} is not visible in DomainInfo yet.");
    ...
    {
        $home = rtrim($home, '/');
        if ($home === '') throw new RuntimeException('cPanel did not return the account home directory.');
        if (str_starts_with($configuredRoot, '/')) {
            $absolute = rtrim($configuredRoot, '/');
            if (! str_starts_with($absolute.'/', $home.'/')) throw new RuntimeException('The configured document root is outside the cPanel account home directory.');
            $relative = ltrim(substr($absolute, strlen($home)), '/');
        } else { $relative = trim($configuredRoot, '/'); $absolute = $home.'/'.$relative; }
        if ($relative === '') throw new RuntimeException('The cPanel addon-domain document root cannot be the account home directory.');
    ...
    private function assertDocumentRoot(string $domain, array $data, string $expectedRoot): void
    {
        ...
        if ($actual === '' || $actual !== $expectedRoot) throw new RuntimeException("The cPanel domain {$domain} exists but is mapped to a different document root. Refusing to change it automatically.");
    }
