...
class CpanelCustomDomainVerifier implements CustomDomainVerifier
{
    ...
    public function verify(string $domain): array
    {
        ...
        if (! $dns) $errors[] = 'DNS does not yet resolve to the platform target.';
        $cpanel = $this->cpanelDomainReady($domain);
        if (! $cpanel) $errors[] = 'The custom domain is not registered in cPanel with the application document root.';
        $ssl = $dns && $cpanel && $this->sslReady($domain);
        if ($dns && $cpanel && ! $ssl) $errors[] = 'HTTPS certificate validation is not ready for this domain.';
    ...
    private function dnsPointsToPlatform(string $domain): bool
    {
        ...
        if ($target === '') $target = strtolower(trim((string) config('central.platform.domain')));
        if ($target === '') return false;
        $domainIps = $this->resolveIpv4($domain); $targetIps = $this->resolveIpv4($target);
    ...
    {
        try { $result = $this->cpanel->uapi('DomainInfo', 'single_domain_data', ['domain' => $domain]); } catch (Throwable) { return false; }
        $data = $result['data'] ?? [];
        if (! is_array($data) || strtolower((string) ($data['domain'] ?? '')) !== $domain) return false;
