...
class CpanelClient
{
    ...
    public function uapi(string $module, string $function, array $arguments = []): array
    {
        ...
        if (! $response->successful()) throw new RuntimeException('cPanel UAPI request failed with HTTP '.$response->status().'.');
        $decoded = $response->json();
        if (! is_array($decoded)) throw new RuntimeException('cPanel UAPI returned an invalid response.');
        ...
        if (! is_array($result)) throw new RuntimeException('cPanel UAPI returned an invalid result.');
        $errors = $this->normalizeErrors($result['errors'] ?? null);
        if ((int) ($result['status'] ?? 0) !== 1 || $errors !== []) throw new RuntimeException($errors !== [] ? implode(' ', $errors) : 'cPanel UAPI call failed.');
        return $result;
    }
    ...
    public function api2(string $module, string $function, array $arguments = []): array
    {
        ...
        if (! $response->successful()) throw new RuntimeException('cPanel API 2 request failed with HTTP '.$response->status().'.');
        $decoded = $response->json();
        $result = is_array($decoded) ? ($decoded['cpanelresult'] ?? null) : null;
        if (! is_array($result)) throw new RuntimeException('cPanel API 2 returned an invalid response.');
        $errors = [];
        if ((int) data_get($result, 'event.result', 0) !== 1) $errors[] = trim((string) ($result['error'] ?? 'cPanel API 2 call failed.'));
        foreach (is_array($result['data'] ?? null) ? $result['data'] : [] as $item) {
            if (is_array($item) && array_key_exists('result', $item) && (int) $item['result'] === 0) $errors[] = trim((string) ($item['reason'] ?? 'cPanel API 2 call failed.'));
        }
    ...
    private function request(): PendingRequest
    {
        ...
        if ($host === '' || $user === '' || $token === '') throw new RuntimeException('cPanel API host, user and token must be configured.');
    ...
    private function baseUrl(): string
    {
        ...
        if ($host === '') throw new RuntimeException('cPanel API host, user and token must be configured.');
        if ($port < 1 || $port > 65535) throw new RuntimeException('cPanel API port is invalid.');
