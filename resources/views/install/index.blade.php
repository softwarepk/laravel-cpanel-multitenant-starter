...
<main>
    ...
    <p class="lead">A guided setup will verify this server, configure the application and databases, and create the first Control Center administrator.</p>
    ...
    <div class="shell">
        <nav class="steps" aria-label="Installation progress">
            @foreach(['Environment', 'Application', 'cPanel', 'Databases', 'Administrator', 'Review'] as $index => $label)
        ...
        <div class="content">
            <form id="installer-form" method="post" action="/install" autocomplete="off" novalidate>
                <section class="card" data-step-panel="1">
                    <div class="section-head"><div><div class="eyebrow">Step 1 of 6</div><h2>Environment check</h2><p class="section-copy">Confirm that Laravel can safely complete setup on this hosting account before credentials or database changes are submitted.</p></div></div>
                    <div class="checks">
                        @foreach ($checks as $check)
                            <div class="check"><span class="dot {{ $check['ok'] ? 'ok' : 'bad' }}"></span><div><strong>{{ $check['label'] }}</strong><span>{{ $check['detail'] }}</span></div></div>
                        @endforeach
                    </div>
                    @if($checksReady)
                    ...
                    @endif
                    <div class="actions"><a class="button secondary" href="/install">Recheck</a><div class="actions-right"><button type="button" data-next {{ $checksReady ? '' : 'disabled' }}>Continue</button></div></div>
                </section>

                <section class="card" data-step-panel="2">
                    <div class="section-head"><div><div class="eyebrow">Step 2 of 6</div><h2>Application</h2><p class="section-copy">Confirm the Control Center hostname, tenant platform root and shared Laravel document root.</p></div></div>
                    <div class="grid">
                ...
                <section class="card" data-step-panel="3">
                    <div class="section-head"><div><div class="eyebrow">Step 3 of 6</div><h2>cPanel connection</h2><p class="section-copy">Use an API token for the cPanel account that manages this deployment. The token is used for the live test and final installation, and is never redisplayed.</p></div></div>
                    <div class="grid">
                ...
                <section class="card" data-step-panel="4">
                    <div class="section-head"><div><div class="eyebrow">Step 4 of 6</div><h2>Databases</h2><p class="section-copy">The Control Center and tenant application use separate database users. Existing users are verified with the passwords supplied; the installer never silently changes an existing database-user password.</p></div></div>
                    ...
                    <div class="grid">
                        ...
                        <div><label for="tenant_db_user">Tenant application DB user</label><input id="tenant_db_user" name="tenant_db_user" type="text" value="{{ $values['tenant_db_user'] ?? '' }}" required><div class="hint">This user receives access to each tenant database as tenants are provisioned.</div>@foreach($errors['tenant_db_user'] ?? [] as $error)<div class="error">{{ $error }}</div>@endforeach</div>
                ...
                <section class="card" data-step-panel="5">
                    <div class="section-head"><div><div class="eyebrow">Step 5 of 6</div><h2>Administrator &amp; options</h2><p class="section-copy">Create the first Control Center administrator and choose the starter's initial tenant-user registration settings.</p></div></div>
                    <div class="grid">
                ...
                <section class="card" data-step-panel="6">
                    <div class="section-head"><div><div class="eyebrow">Step 6 of 6</div><h2>Review &amp; install</h2><p class="section-copy">Review the non-secret deployment details before the installer creates or verifies cPanel resources and writes the production environment.</p></div></div>
                    <dl class="summary">
                        ...
                        <dt>Tenant lifecycle</dt><dd>Synchronous cPanel operations; no queue worker or cron required</dd>
...
(() => {
    ...
    document.querySelectorAll('[data-next]').forEach((button) => {
        button.addEventListener('click', () => {
            ...
            if (step === 3 && !cpanelVerified) return;
            if (step === 4 && !databaseVerified) return;
    ...
    const databaseFields = ['db_host', 'db_port', 'central_db_name', 'central_db_user', 'central_db_password', 'tenant_db_host', 'tenant_db_port', 'tenant_db_user', 'tenant_db_password', 'tenant_db_prefix'];
    ...
    databaseFields.forEach((name) => field(name)?.addEventListener('input', resetDatabaseVerification));
    ...
    databaseTest?.addEventListener('click', async () => {
        ...
        if (!cpanelVerified) {
            if (databaseStatus) {
                ...
                databaseStatus.textContent = 'Test the cPanel connection again before verifying databases.';
            }
            return;
        }
        ...
        if (databaseStatus) {
            databaseStatus.className = 'connection-status';
            databaseStatus.textContent = 'Checking database names, users, credentials and existing central database contents…';
        }
    ...
    form.addEventListener('submit', (event) => {
        const review = document.querySelector('[data-step-panel="6"]');
